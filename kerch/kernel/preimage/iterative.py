...
def iterative(k_coefficient: torch.Tensor, kernel: K, num_iter: int = 100, lr=1.e-3, lightweight_cache=True) -> torch.Tensor:
    ...
    if cache_level > _Cache._cache_level_switcher['lightweight']:
        if lightweight_cache:
            ...
            kernel._log.warn("The cache level is recommended to be at lightweight at maximum in order to ease the "
                             "memory load during the pre-image computation. It is temporarily being set to "
                             "lightweight. You can also set the argument lightweight_cache to True to set it "
