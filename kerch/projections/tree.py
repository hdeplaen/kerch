...
from ._mean_centering import _MeanCentering
from ._sphere import _UnitSphereNormalization
from ._variance import _UnitVarianceNormalization
from ._minmax import _MinMaxNormalization


@kerch.utils.extend_docstring(_Projection)
class ProjectionTree(_Projection):
    r"""
    Creates a tree of projections for efficient computing, with cache management.

    :param explicit: True is the projections are to be computed in the explicit formulation, False instead.
    :param sample: Default sample on which the statistics are to be computed.
    :param default_projections: Optional default list of projections., defaults to None.
    ...
    def beautify_projections(projections) -> Union[None, List[_Projection]]:
        ...
        :param projections: list of the different projections.
    ...
    def default_projections(self) -> List:
        r"""
        Default list of projections to be applied.
        """
    ...
    def apply(self, oos=None, x=None, y=None, projections: List[str] = None) -> Tensor:
        r"""
        Applies the projections to the value to out-of-sample data. Either value is a function handle and you can use
        x (explicit) and x, y (explicit) to specify the data. Either directly give a Tensor.

        .. warning::
            If value is a Tensor, some projections may not work in implicit formulation. For example, the unit sphere
        ...
        :param projections: Projections to be used. If none are to be used, i.e. getting the raw data back, please
            specify [], not None, which will return the default projections used for the sample., defaults to None,
            i.e., the default projections.
    ...
    def revert(self, value, projections: List[str] = None) -> Tensor:
        r"""
        Reverts the projections (runs the tree backwards) to the value to out-of-sample data.
