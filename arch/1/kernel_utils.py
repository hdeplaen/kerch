...
class Hilbert(nn.Module):
    def __init__(self, size_in, num_kernels, size_out, sigma, kernel_type, sigma_trainable, points_trainable):
        ...
        self.model = nn.ModuleDict({
            ...
            'expkernel': ExponentialKernel(self.size_in, self.num_kernels, self.sigma, self.sigma_trainable, self.points_trainable),
            'lin': CustomLinear(self.num_kernels, self.size_out)
        })

    def forward(self, x):
        x = self.model[self.kernel_type](x)
        x = self.model['lin'](x)
        return x
    ...
    def kernel_matrix(self, idx_sv):
        sv = self.model[self.kernel_type].param[:, idx_sv].t()
        K = self.model[self.kernel_type].forward(sv, idx_sv)
        return K

    def regularization_term(self, idx_sv=None):
        if idx_sv is None:
            idx_sv = range(self.num_kernels)

        alpha = self.model['lin'].weight[idx_sv] # problem here (not differentiable)
        K = self.kernel_matrix(idx_sv)
        reg = alpha @ K @ alpha.t()
...
class CustomLinear(nn.Module):
    ...
    def forward(self, x):
        return x @ self.weight + self.bias.expand(x.size(0))
...
class ExponentialKernel(nn.Module):
    def __init__(self, size_in, num_kernels, sigma, sigma_trainable=False, points_trainable=True):
        ...
        self.param = nn.Parameter(4 * nn.init.orthogonal_(torch.Tensor(self.size_in, self.num_kernels)), requires_grad=points_trainable)
