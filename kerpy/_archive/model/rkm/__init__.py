...
import kerpy
from kerpy.utils import process_y
import kerpy._archive.model.level.IDXK as IDXK
import kerpy._archive.model.opt as Optimizer
import kerpy._archive.model.lssvm.SoftLSSVM as SoftLSSVM
import kerpy._archive.model.lssvm.HardLSSVM as HardLSSVM
import kerpy._archive.model.kpca.SoftKPCA as SoftKPCA
import kerpy._archive.model.kpca.HardKPCA as HardKPCA
import kerpy._archive.plot as rkmplot


class RKM(torch.nn.Module):
    @kerpy.kwargs_decorator(
        {"cuda": True,
         "verbose": True,
         "name":'noname'})
    ...
    def learn(self, x, y, verbose=False, val_x=None, val_y=None, test_x=None, test_y=None, **kwargs):
        ...
        if val:
            val_x = torch.tensor(val_x, dtype=kerpy.ftype)
            val_y = torch.tensor(val_y, dtype=kerpy.ftype)
            val_y = process_y(val_y)
        if test:
            test_x = torch.tensor(test_x, dtype=kerpy.ftype)
            test_y = torch.tensor(test_y, dtype=kerpy.ftype)
        ...
        x = torch.tensor(x, dtype=kerpy.ftype)
        y = torch.tensor(y, dtype=kerpy.ftype)
        y = process_y(y)
    ...
    def evaluate(self, x, numpy=True):
        if not torch.is_tensor(x): x = torch.tensor(x, dtype=kerpy.ftype).to(self.device)
        self.to(self.device)
        for level in self.model:
            x = level.evaluate(x)
    ...
    @kerpy.kwargs_decorator({"size_in": 1, "constraint": "soft", "representation": "dual"})
