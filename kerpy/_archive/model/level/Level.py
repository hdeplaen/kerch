...
import kerpy.kernel.factory as KernelFactory
import kerpy._archive.model.level.DualLinear as DualLinear
import kerpy._archive.model.level.PrimalLinear as PrimalLinear
...
class Level(torch.nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self, device='cpu', **kwargs):
        ...
        self._y = torch.nn.Parameter(torch.ones((kwargs["init_kernels"], kwargs["size_out"]), dtype=kerpy.ftype),
                                     requires_grad=False)
    ...
    def _generate_model(self, **kwargs):
        ...
        if kwargs["representation"] not in switcher:
            raise kerpy.model.RepresentationError
    ...
    @kerpy.kwargs_decorator({"mtol": 1.0e-2, "rtol": 1.0e-4})
